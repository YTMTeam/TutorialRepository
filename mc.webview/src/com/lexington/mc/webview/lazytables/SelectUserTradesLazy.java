...
public class SelectUserTradesLazy extends LazyDataModel<EntityVPaymentDeprecated>implements SelectableDataModel<EntityVPaymentDeprecated>{
	 private static final long  serialVersionUID    = -6018436278490045301L;
	  ...
	  private int rowCntBuy=0;
	   ...
	   public SelectUserTradesLazy(String userId) {
		   this.userId = userId;
	   }
	   ...
	   public List<EntityVPaymentDeprecated> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
	      ...
	      try {
	    	  ...
	    	  result = paymentProvider.getUserPaymentsForPaging(first, pageSize, sortField, dbUtil.translateSortOrder(sortOrder), filters , userId);
	      ...
	      try {
	         int rowCnt = (int) paymentProvider.getUserPaymentsPaymentsFilterCount(filters, userId);
	         filters.put("DIRECTION", BlockchainConstants.DirectionStatus.SELL);
	            rowCntSell = (int) paymentProvider.getUserPaymentsPaymentsFilterCount(filters, userId);
