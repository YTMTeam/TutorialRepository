 ...
 *  <pre>
 *
 *  Generated Class: POSITIONExcelSample.java
 * 
...
public class PositionExcelSample {

   final static Logger log = Logger.getLogger(PositionExcelSample.class.getName());
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         PositionExcelSample positionExcelSample = new PositionExcelSample();
          ...
          *
          **********************************************************************/
         positionExcelSample.createExcelFileAsByteArray(connection, testFile);
         
         PositionTab positionTab = new PositionTab(connection);
         positionTab.deleteAllRecords();
         positionExcelSample.loadDataFromExcelFile(connection, testFile);
         
         //positionExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);
   ...
   private byte[] createExcelFileAsByteArray(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"saveDataToExcelFile()\" for TABLE \"POSITION\"";
      ...
      try {
         ...
         if (!positionTab.exists()) {
            String errMessage = "TABLE " + positionTab.getDbEntityName() + " does not exist.";
            log.error(errMessage);
            throw new GDAOException(errMessage);
         } else {
            log.debug("TABLE " + positionTab.getDbEntityName() + " exists.");
         }
          ...
          *
          **********************************************************************/
         //positionTab.getResultSet("WHERE UPPER(CREDIT_DEBIT_CODE) LIKE ? AND ROWNUM < ?", "%A%", 100);
          ...
          * 
          **********************************************************************/
         //positionTabb.getResultSet(null, (Object[])null);
         ...
         XSSFCell POSITION_ID_cell = rowHeader.createCell(0);
POSITION_ID_cell.setCellValue("POSITION ID");
POSITION_ID_cell.setCellStyle(getStyle("Header", workbook));
...
XSSFCell BROKER_CODE_cell = rowHeader.createCell(2);
BROKER_CODE_cell.setCellValue("BROKER CODE");
BROKER_CODE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell SECURITY_SYMBOL_cell = rowHeader.createCell(3);
SECURITY_SYMBOL_cell.setCellValue("SECURITY SYMBOL");
SECURITY_SYMBOL_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell CUSIP_cell = rowHeader.createCell(4);
CUSIP_cell.setCellValue("CUSIP");
CUSIP_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell ISIN_cell = rowHeader.createCell(5);
ISIN_cell.setCellValue("ISIN");
ISIN_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell SEDOL_cell = rowHeader.createCell(6);
...
XSSFCell EXPIRATION_DATE_cell = rowHeader.createCell(8);
EXPIRATION_DATE_cell.setCellValue("EXPIRATION DATE");
EXPIRATION_DATE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell EXECUTION_PRICE_cell = rowHeader.createCell(9);
EXECUTION_PRICE_cell.setCellValue("EXECUTION PRICE");
EXECUTION_PRICE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell MARKET_PRICE_cell = rowHeader.createCell(10);
MARKET_PRICE_cell.setCellValue("MARKET PRICE");
MARKET_PRICE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell QUANTITY_cell = rowHeader.createCell(11);
...
QUANTITY_cell.setCellStyle(getStyle("Header", workbook));
         ...
         while (positionTab.next()) {
            ...
            addStringCell(workbook, currentRow, column++, row, positionTab.getPOSITION_ID());
addStringCell(workbook, currentRow, column++, row, positionTab.getACCOUNT_ID());
addStringCell(workbook, currentRow, column++, row, positionTab.getBROKER_CODE());
addStringCell(workbook, currentRow, column++, row, positionTab.getSECURITY_SYMBOL());
addStringCell(workbook, currentRow, column++, row, positionTab.getCUSIP());
addStringCell(workbook, currentRow, column++, row, positionTab.getISIN());
addStringCell(workbook, currentRow, column++, row, positionTab.getSEDOL());
addStringCell(workbook, currentRow, column++, row, positionTab.getCURRENCY());
addTimestampCell(workbook, currentRow, column++, row, positionTab.getEXPIRATION_DATE());
addNumberCell(workbook, currentRow, column++, row, positionTab.getEXECUTION_PRICE());
addNumberCell(workbook, currentRow, column++, row, positionTab.getMARKET_PRICE());
addNumberCell(workbook, currentRow, column++, row, positionTab.getQUANTITY());
...
sheet.autoSizeColumn(11);
         ...
         try {
            ...
            e.printStackTrace();  
            log.error("Cannot write workbook to byte[] output stream for TABLE \"POSITION\".");
         ...
         try {
            ...
            log.error("Cannot close workbook for TABLE \"POSITION\".");
            e.printStackTrace();
            throw new GDAOException(e);
         }
      }
   ...
   private void loadDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException{
      ...
      String whatWeAreDoing = "\"loadDataFromExcelFile()\" for TABLE \"POSITION\"";
      log.debug("Starting " + whatWeAreDoing);
         ...
         if (rows > 0 && columns > 0) {
            for (row = 1; row < rows; row++) { // skip first row since it is header row
                  ...
                  int rowColumn = 0;
                        ...
                        positionTab.setPOSITION_ID(getString(currentRow.getCell(0)));
      positionTab.setACCOUNT_ID(getString(currentRow.getCell(1)));
      positionTab.setBROKER_CODE(getString(currentRow.getCell(2)));
      positionTab.setSECURITY_SYMBOL(getString(currentRow.getCell(3)));
      positionTab.setCUSIP(getString(currentRow.getCell(4)));
      positionTab.setISIN(getString(currentRow.getCell(5)));
      positionTab.setSEDOL(getString(currentRow.getCell(6)));
      positionTab.setCURRENCY(getString(currentRow.getCell(7)));
      positionTab.setEXPIRATION_DATE(getTimestamp(currentRow.getCell(8)));
      positionTab.setEXECUTION_PRICE(getNumber(currentRow.getCell(9)));
      positionTab.setMARKET_PRICE(getNumber(currentRow.getCell(10)));
      positionTab.setQUANTITY(getNumber(currentRow.getCell(11)));
         ...
         log.info("Inserted Records in TABLE POSITION DB: " + inserted);
         log.info("Total errors: " + errors);
         log.info("Total row count in TABLE \"POSITION\" is " + rowCount);
   ...
   private void updateOrInsertDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"updateOrInsertDataFromExcelFile()\" for TABLE \"POSITION\"";
      log.debug("Starting " + whatWeAreDoing);
      ...
      positionTab.setQUANTITY(getNumber(currentRow.getCell(11)));
                ...
                **********************************************************************/
               try {
                  recordsUpdated = positionTab.updateThisRecord();
               } catch (GDAOException e) {
         ...
         log.info("Inserted Records in TABLE POSITION DB: " + inserted);
         log.info("Updated Records in TABLE POSITION DB: " + updated);
         log.info("Total errors: " + errors);
         log.info("Row Count (TABLE POSITION) =" + rowCount);
