 ...
 *  <pre>
 *
 *  Generated Class: TRANSACTIONExcelSample.java
 * 
...
public class TransactionExcelSample {

   final static Logger log = Logger.getLogger(TransactionExcelSample.class.getName());

   final static String ENTITY_NAME = "TRANSACTION";
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         TransactionExcelSample transactionExcelSample = new TransactionExcelSample();
          ...
          *
          **********************************************************************/
         transactionExcelSample.createExcelFileAsByteArray(connection, testFile);
         
         TransactionTab transactionTab = new TransactionTab(connection);
         transactionTab.deleteAllRecords();
         transactionExcelSample.loadDataFromExcelFile(connection, testFile);
         
         //transactionExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);
   ...
   private byte[] createExcelFileAsByteArray(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"saveDataToExcelFile()\" for TABLE \"TRANSACTION\"";
      ...
      try {
         ...
         if (!transactionTab.exists()) {
            String errMessage = "TABLE " + transactionTab.getDbEntityName() + " does not exist.";
            log.error(errMessage);
            throw new GDAOException(errMessage);
         } else {
            log.debug("TABLE " + transactionTab.getDbEntityName() + " exists.");
         }
          ...
          *
          **********************************************************************/
         //transactionTab.getResultSet("WHERE UPPER(CREDIT_DEBIT_CODE) LIKE ? AND ROWNUM < ?", "%A%", 100);
          ...
          * 
          **********************************************************************/
         //transactionTabb.getResultSet(null, (Object[])null);
         ...
         XSSFCell TRANSACTION_ID_cell = rowHeader.createCell(0);
TRANSACTION_ID_cell.setCellValue("TRANSACTION ID");
TRANSACTION_ID_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell BLOCK_ID_cell = rowHeader.createCell(1);
BLOCK_ID_cell.setCellValue("BLOCK ID");
BLOCK_ID_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell TRANSACTION_HASH_cell = rowHeader.createCell(2);
TRANSACTION_HASH_cell.setCellValue("TRANSACTION HASH");
TRANSACTION_HASH_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell VERSION_cell = rowHeader.createCell(3);
VERSION_cell.setCellValue("VERSION");
VERSION_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell LOCKTIME_cell = rowHeader.createCell(4);
LOCKTIME_cell.setCellValue("LOCKTIME");
LOCKTIME_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell CREATE_TIMESTAMP_cell = rowHeader.createCell(5);
CREATE_TIMESTAMP_cell.setCellValue("CREATE TIMESTAMP");
CREATE_TIMESTAMP_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell TYPE_cell = rowHeader.createCell(6);
TYPE_cell.setCellValue("TYPE");
TYPE_cell.setCellStyle(getStyle("Header", workbook));
         ...
         while (transactionTab.next()) {
            ...
            addNumberCell(workbook, currentRow, column++, row, transactionTab.getTRANSACTION_ID());
addNumberCell(workbook, currentRow, column++, row, transactionTab.getBLOCK_ID());
addStringCell(workbook, currentRow, column++, row, transactionTab.getTRANSACTION_HASH());
addNumberCell(workbook, currentRow, column++, row, transactionTab.getVERSION());
addNumberCell(workbook, currentRow, column++, row, transactionTab.getLOCKTIME());
addTimestampCell(workbook, currentRow, column++, row, transactionTab.getCREATE_TIMESTAMP());
addStringCell(workbook, currentRow, column++, row, transactionTab.getTYPE());
...
sheet.autoSizeColumn(6);
         ...
         try {
            ...
            e.printStackTrace();  
            log.error("Cannot write workbook to byte[] output stream for TABLE \"TRANSACTION\".");
         ...
         try {
            ...
            log.error("Cannot close workbook for TABLE \"TRANSACTION\".");
            e.printStackTrace();
            throw new GDAOException(e);
         }
      }
   ...
   private void loadDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException{
      ...
      String whatWeAreDoing = "\"loadDataFromExcelFile()\" for TABLE \"TRANSACTION\"";
      log.debug("Starting " + whatWeAreDoing);
         ...
         if (rows > 0 && columns > 0) {
            for (row = 1; row < rows; row++) { // skip first row since it is header row
                  ...
                  int rowColumn = 0;
                        ...
                        transactionTab.setTRANSACTION_ID(getNumber(currentRow.getCell(0)));
      transactionTab.setBLOCK_ID(getNumber(currentRow.getCell(1)));
      transactionTab.setTRANSACTION_HASH(getString(currentRow.getCell(2)));
      transactionTab.setVERSION(getNumber(currentRow.getCell(3)));
      transactionTab.setLOCKTIME(getNumber(currentRow.getCell(4)));
      transactionTab.setCREATE_TIMESTAMP(getTimestamp(currentRow.getCell(5)));
      transactionTab.setTYPE(getString(currentRow.getCell(6)));
         ...
         long rowCount = transactionTab.getRowCount();

         log.info("Inserted Records in TABLE TRANSACTION DB: " + inserted);
         log.info("Total errors: " + errors);
         log.info("Total row count in TABLE \"TRANSACTION\" is " + rowCount);
   ...
   private void updateOrInsertDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"updateOrInsertDataFromExcelFile()\" for TABLE \"TRANSACTION\"";
      log.debug("Starting " + whatWeAreDoing);
      ...
      transactionTab.setTYPE(getString(currentRow.getCell(6)));
                ...
                **********************************************************************/
               try {
                  recordsUpdated = transactionTab.updateThisRecord();
               } catch (GDAOException e) {
         ...
         log.info("Inserted Records in TABLE TRANSACTION DB: " + inserted);
         log.info("Updated Records in TABLE TRANSACTION DB: " + updated);
         log.info("Total errors: " + errors);
         log.info("Row Count (TABLE TRANSACTION) =" + rowCount);
