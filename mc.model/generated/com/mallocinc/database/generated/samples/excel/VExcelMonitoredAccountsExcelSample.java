 ...
 *  <pre>
 *
 *  Generated Class: V_EXCEL_MONITORED_ACCOUNTSExcelSample.java
 * 
...
public class VExcelMonitoredAccountsExcelSample {

   final static Logger log = Logger.getLogger(VExcelMonitoredAccountsExcelSample.class.getName());

   final static String ENTITY_NAME = "V_EXCEL_MONITORED_ACCOUNTS";
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         VExcelMonitoredAccountsExcelSample vExcelMonitoredAccountsExcelSample = new VExcelMonitoredAccountsExcelSample();
          ...
          *
          **********************************************************************/
         vExcelMonitoredAccountsExcelSample.createExcelFileAsByteArray(connection, testFile);
         
         VExcelMonitoredAccountsTab vExcelMonitoredAccountsTab = new VExcelMonitoredAccountsTab(connection);
         vExcelMonitoredAccountsTab.deleteAllRecords();
         vExcelMonitoredAccountsExcelSample.loadDataFromExcelFile(connection, testFile);
         
         //vExcelMonitoredAccountsExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);

         //V_EXCEL_MONITORED_ACCOUNTSExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);
   ...
   private byte[] createExcelFileAsByteArray(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"saveDataToExcelFile()\" for VIEW \"V_EXCEL_MONITORED_ACCOUNTS\"";
      ...
      try {
         XSSFSheet sheet = workbook.createSheet("V_EXCEL_MONITORED_ACCOUNTS");

         if (!vExcelMonitoredAccountsTab.exists()) {
            String errMessage = "VIEW " + vExcelMonitoredAccountsTab.getDbEntityName() + " does not exist.";
            log.error(errMessage);
            throw new GDAOException(errMessage);
         } else {
            log.debug("VIEW " + vExcelMonitoredAccountsTab.getDbEntityName() + " exists.");
         }
          ...
          *
          **********************************************************************/
         //vExcelMonitoredAccountsTab.getResultSet("WHERE UPPER(CREDIT_DEBIT_CODE) LIKE ? AND ROWNUM < ?", "%A%", 100);
          ...
          * 
          **********************************************************************/
         //vExcelMonitoredAccountsTabb.getResultSet(null, (Object[])null);
...
XSSFCell MONITOR_BANK_ID_cell = rowHeader.createCell(1);
...
BANK_NAME_cell.setCellStyle(getStyle("Header", workbook));
         ...
         while (vExcelMonitoredAccountsTab.next()) {
            ...
            addStringCell(workbook, currentRow, column++, row, vExcelMonitoredAccountsTab.getACCOUNT_ID());
addNumberCell(workbook, currentRow, column++, row, vExcelMonitoredAccountsTab.getMONITOR_BANK_ID());
addNumberCell(workbook, currentRow, column++, row, vExcelMonitoredAccountsTab.getACCOUNT_NUMBER());
addStringCell(workbook, currentRow, column++, row, vExcelMonitoredAccountsTab.getFIRST_NAME());
addStringCell(workbook, currentRow, column++, row, vExcelMonitoredAccountsTab.getLAST_NAME());
addStringCell(workbook, currentRow, column++, row, vExcelMonitoredAccountsTab.getBANK_NAME());
...
sheet.autoSizeColumn(5);
         ...
         try {
            ...
            e.printStackTrace();  
            log.error("Cannot write workbook to byte[] output stream for VIEW \"V_EXCEL_MONITORED_ACCOUNTS\".");
         ...
         try {
            ...
            log.error("Cannot close workbook for VIEW \"V_EXCEL_MONITORED_ACCOUNTS\".");
            e.printStackTrace();
            throw new GDAOException(e);
         }
      }
    ...
    *  VIEW \"V_EXCEL_MONITORED_ACCOUNTS\". 
    *
   ...
   private void loadDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException{
      ...
      String whatWeAreDoing = "\"loadDataFromExcelFile()\" for VIEW \"V_EXCEL_MONITORED_ACCOUNTS\"";
      log.debug("Starting " + whatWeAreDoing);
         ...
         if (rows > 0 && columns > 0) {
            for (row = 1; row < rows; row++) { // skip first row since it is header row
                  ...
                  int rowColumn = 0;
                        ...
                        vExcelMonitoredAccountsTab.setACCOUNT_ID(getString(currentRow.getCell(0)));
      vExcelMonitoredAccountsTab.setMONITOR_BANK_ID(getNumber(currentRow.getCell(1)));
      vExcelMonitoredAccountsTab.setACCOUNT_NUMBER(getNumber(currentRow.getCell(2)));
      vExcelMonitoredAccountsTab.setFIRST_NAME(getString(currentRow.getCell(3)));
      vExcelMonitoredAccountsTab.setLAST_NAME(getString(currentRow.getCell(4)));
      vExcelMonitoredAccountsTab.setBANK_NAME(getString(currentRow.getCell(5)));
         ...
         log.info("Inserted Records in VIEW V_EXCEL_MONITORED_ACCOUNTS DB: " + inserted);
         log.info("Total errors: " + errors);
         log.info("Total row count in VIEW \"V_EXCEL_MONITORED_ACCOUNTS\" is " + rowCount);
   ...
   private void updateOrInsertDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"updateOrInsertDataFromExcelFile()\" for VIEW \"V_EXCEL_MONITORED_ACCOUNTS\"";
      log.debug("Starting " + whatWeAreDoing);
      ...
      vExcelMonitoredAccountsTab.setBANK_NAME(getString(currentRow.getCell(5)));
                ...
                **********************************************************************/
               try {
                  recordsUpdated = vExcelMonitoredAccountsTab.updateThisRecord();
               } catch (GDAOException e) {
         ...
         log.info("Inserted Records in VIEW V_EXCEL_MONITORED_ACCOUNTS DB: " + inserted);
         log.info("Updated Records in VIEW V_EXCEL_MONITORED_ACCOUNTS DB: " + updated);
         log.info("Total errors: " + errors);
         log.info("Row Count (VIEW V_EXCEL_MONITORED_ACCOUNTS) =" + rowCount);
