 ...
 *  <pre>
 *
 *  Generated Class: BANKExcelSample.java
 * 
...
public class BankExcelSample {

   final static Logger log = Logger.getLogger(BankExcelSample.class.getName());

   final static String ENTITY_NAME = "BANK";
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         BankExcelSample bankExcelSample = new BankExcelSample();
          ...
          *
          **********************************************************************/
         bankExcelSample.createExcelFileAsByteArray(connection, testFile);
         
         BankTab bankTab = new BankTab(connection);
         bankTab.deleteAllRecords();
         bankExcelSample.loadDataFromExcelFile(connection, testFile);
         
         //bankExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);
   ...
   private byte[] createExcelFileAsByteArray(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"saveDataToExcelFile()\" for TABLE \"BANK\"";
      ...
      try {
         ...
         if (!bankTab.exists()) {
            String errMessage = "TABLE " + bankTab.getDbEntityName() + " does not exist.";
            log.error(errMessage);
            throw new GDAOException(errMessage);
         } else {
            log.debug("TABLE " + bankTab.getDbEntityName() + " exists.");
         }
          ...
          *
          **********************************************************************/
         //bankTab.getResultSet("WHERE UPPER(CREDIT_DEBIT_CODE) LIKE ? AND ROWNUM < ?", "%A%", 100);
          ...
          * 
          **********************************************************************/
         //bankTabb.getResultSet(null, (Object[])null);
         ...
         XSSFCell BANK_ID_cell = rowHeader.createCell(0);
BANK_ID_cell.setCellValue("BANK ID");
BANK_ID_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell BANK_CODE_cell = rowHeader.createCell(1);
BANK_CODE_cell.setCellValue("BANK CODE");
BANK_CODE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell BANK_NAME_cell = rowHeader.createCell(2);
BANK_NAME_cell.setCellValue("BANK NAME");
BANK_NAME_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell BANK_LOGO_cell = rowHeader.createCell(3);
BANK_LOGO_cell.setCellValue("BANK LOGO");
BANK_LOGO_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell KEY_CODE_cell = rowHeader.createCell(4);
KEY_CODE_cell.setCellValue("KEY CODE");
KEY_CODE_cell.setCellStyle(getStyle("Header", workbook));
         ...
         while (bankTab.next()) {
            ...
            addNumberCell(workbook, currentRow, column++, row, bankTab.getBANK_ID());
addStringCell(workbook, currentRow, column++, row, bankTab.getBANK_CODE());
addStringCell(workbook, currentRow, column++, row, bankTab.getBANK_NAME());
column++;
addStringCell(workbook, currentRow, column++, row, bankTab.getKEY_CODE());
...
sheet.autoSizeColumn(4);
         ...
         try {
            ...
            e.printStackTrace();  
            log.error("Cannot write workbook to byte[] output stream for TABLE \"BANK\".");
         ...
         try {
            ...
            log.error("Cannot close workbook for TABLE \"BANK\".");
            e.printStackTrace();
            throw new GDAOException(e);
         }
      }
   ...
   private void loadDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException{
      ...
      String whatWeAreDoing = "\"loadDataFromExcelFile()\" for TABLE \"BANK\"";
      log.debug("Starting " + whatWeAreDoing);
         ...
         if (rows > 0 && columns > 0) {
            for (row = 1; row < rows; row++) { // skip first row since it is header row
                  ...
                  int rowColumn = 0;
                        ...
                        bankTab.setBANK_ID(getNumber(currentRow.getCell(0)));
      bankTab.setBANK_CODE(getString(currentRow.getCell(1)));
      bankTab.setBANK_NAME(getString(currentRow.getCell(2)));
      bankTab.setKEY_CODE(getString(currentRow.getCell(3)));
         ...
         long rowCount = bankTab.getRowCount();

         log.info("Inserted Records in TABLE BANK DB: " + inserted);
         log.info("Total errors: " + errors);
         log.info("Total row count in TABLE \"BANK\" is " + rowCount);
   ...
   private void updateOrInsertDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"updateOrInsertDataFromExcelFile()\" for TABLE \"BANK\"";
      log.debug("Starting " + whatWeAreDoing);
      ...
      bankTab.setKEY_CODE(getString(currentRow.getCell(3)));
                ...
                **********************************************************************/
               try {
                  recordsUpdated = bankTab.updateThisRecord();
               } catch (GDAOException e) {
         ...
         log.info("Inserted Records in TABLE BANK DB: " + inserted);
         log.info("Updated Records in TABLE BANK DB: " + updated);
         log.info("Total errors: " + errors);
         log.info("Row Count (TABLE BANK) =" + rowCount);
