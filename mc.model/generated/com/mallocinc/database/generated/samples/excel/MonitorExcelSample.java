 ...
 *  <pre>
 *
 *  Generated Class: MONITORExcelSample.java
 * 
...
public class MonitorExcelSample {

   final static Logger log = Logger.getLogger(MonitorExcelSample.class.getName());

   final static String ENTITY_NAME = "MONITOR";
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         MonitorExcelSample monitorExcelSample = new MonitorExcelSample();
          ...
          *
          **********************************************************************/
         monitorExcelSample.createExcelFileAsByteArray(connection, testFile);
         
         MonitorTab monitorTab = new MonitorTab(connection);
         monitorTab.deleteAllRecords();
         monitorExcelSample.loadDataFromExcelFile(connection, testFile);
         
         //monitorExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);
   ...
   private byte[] createExcelFileAsByteArray(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"saveDataToExcelFile()\" for TABLE \"MONITOR\"";
      ...
      try {
         XSSFSheet sheet = workbook.createSheet("MONITOR");

         if (!monitorTab.exists()) {
            String errMessage = "TABLE " + monitorTab.getDbEntityName() + " does not exist.";
            log.error(errMessage);
            throw new GDAOException(errMessage);
         } else {
            log.debug("TABLE " + monitorTab.getDbEntityName() + " exists.");
         }
          ...
          *
          **********************************************************************/
         //monitorTab.getResultSet("WHERE UPPER(CREDIT_DEBIT_CODE) LIKE ? AND ROWNUM < ?", "%A%", 100);
          ...
          * 
          **********************************************************************/
         //monitorTabb.getResultSet(null, (Object[])null);
...
XSSFCell BANK_ID_cell = rowHeader.createCell(2);
BANK_ID_cell.setCellValue("BANK ID");
BANK_ID_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell EMPLOYEE_ID_cell = rowHeader.createCell(3);
...
XSSFCell ACCOUNT_MIDDLE_NAME_cell = rowHeader.createCell(5);
ACCOUNT_MIDDLE_NAME_cell.setCellValue("ACCOUNT MIDDLE NAME");
ACCOUNT_MIDDLE_NAME_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell ACCOUNT_LAST_NAME_cell = rowHeader.createCell(6);
ACCOUNT_LAST_NAME_cell.setCellValue("ACCOUNT LAST NAME");
ACCOUNT_LAST_NAME_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell RELATIONSHIP_cell = rowHeader.createCell(7);
RELATIONSHIP_cell.setCellValue("RELATIONSHIP");
RELATIONSHIP_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell AS_OF_DATE_cell = rowHeader.createCell(8);
AS_OF_DATE_cell.setCellValue("AS OF DATE");
AS_OF_DATE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell END_DATE_cell = rowHeader.createCell(9);
END_DATE_cell.setCellValue("END DATE");
END_DATE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell CREATE_TIMESTAMP_cell = rowHeader.createCell(10);
...
CREATE_TIMESTAMP_cell.setCellStyle(getStyle("Header", workbook));
         ...
         while (monitorTab.next()) {
            ...
            addStringCell(workbook, currentRow, column++, row, monitorTab.getMONITOR_ID());
addStringCell(workbook, currentRow, column++, row, monitorTab.getACCOUNT_ID());
addNumberCell(workbook, currentRow, column++, row, monitorTab.getBANK_ID());
addStringCell(workbook, currentRow, column++, row, monitorTab.getEMPLOYEE_ID());
addStringCell(workbook, currentRow, column++, row, monitorTab.getACCOUNT_FIRST_NAME());
addStringCell(workbook, currentRow, column++, row, monitorTab.getACCOUNT_MIDDLE_NAME());
addStringCell(workbook, currentRow, column++, row, monitorTab.getACCOUNT_LAST_NAME());
addStringCell(workbook, currentRow, column++, row, monitorTab.getRELATIONSHIP());
addTimestampCell(workbook, currentRow, column++, row, monitorTab.getAS_OF_DATE());
addTimestampCell(workbook, currentRow, column++, row, monitorTab.getEND_DATE());
addTimestampCell(workbook, currentRow, column++, row, monitorTab.getCREATE_TIMESTAMP());
...
sheet.autoSizeColumn(10);
         ...
         try {
            ...
            e.printStackTrace();  
            log.error("Cannot write workbook to byte[] output stream for TABLE \"MONITOR\".");
         ...
         try {
            ...
            log.error("Cannot close workbook for TABLE \"MONITOR\".");
            e.printStackTrace();
            throw new GDAOException(e);
         }
      }
   ...
   private void loadDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException{
      ...
      String whatWeAreDoing = "\"loadDataFromExcelFile()\" for TABLE \"MONITOR\"";
      log.debug("Starting " + whatWeAreDoing);
         ...
         if (rows > 0 && columns > 0) {
            for (row = 1; row < rows; row++) { // skip first row since it is header row
                  ...
                  int rowColumn = 0;
                        ...
                        monitorTab.setMONITOR_ID(getString(currentRow.getCell(0)));
      monitorTab.setACCOUNT_ID(getString(currentRow.getCell(1)));
      monitorTab.setBANK_ID(getNumber(currentRow.getCell(2)));
      monitorTab.setEMPLOYEE_ID(getString(currentRow.getCell(3)));
      monitorTab.setACCOUNT_FIRST_NAME(getString(currentRow.getCell(4)));
      monitorTab.setACCOUNT_MIDDLE_NAME(getString(currentRow.getCell(5)));
      monitorTab.setACCOUNT_LAST_NAME(getString(currentRow.getCell(6)));
      monitorTab.setRELATIONSHIP(getString(currentRow.getCell(7)));
      monitorTab.setAS_OF_DATE(getTimestamp(currentRow.getCell(8)));
      monitorTab.setEND_DATE(getTimestamp(currentRow.getCell(9)));
      monitorTab.setCREATE_TIMESTAMP(getTimestamp(currentRow.getCell(10)));
         ...
         long rowCount = monitorTab.getRowCount();

         log.info("Inserted Records in TABLE MONITOR DB: " + inserted);
         log.info("Total errors: " + errors);
         log.info("Total row count in TABLE \"MONITOR\" is " + rowCount);
   ...
   private void updateOrInsertDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"updateOrInsertDataFromExcelFile()\" for TABLE \"MONITOR\"";
      log.debug("Starting " + whatWeAreDoing);
      ...
      monitorTab.setCREATE_TIMESTAMP(getTimestamp(currentRow.getCell(10)));
                ...
                **********************************************************************/
               try {
                  recordsUpdated = monitorTab.updateThisRecord();
               } catch (GDAOException e) {
         ...
         log.info("Inserted Records in TABLE MONITOR DB: " + inserted);
         log.info("Updated Records in TABLE MONITOR DB: " + updated);
         log.info("Total errors: " + errors);
         log.info("Row Count (TABLE MONITOR) =" + rowCount);
