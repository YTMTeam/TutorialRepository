 ...
 *  <pre>
 *
 *  Generated Class: V_TRADEExcelSample.java
 * 
...
public class VTradeExcelSample {

   final static Logger log = Logger.getLogger(VTradeExcelSample.class.getName());

   final static String ENTITY_NAME = "V_TRADE";
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         VTradeExcelSample vTradeExcelSample = new VTradeExcelSample();
          ...
          *
          **********************************************************************/
         vTradeExcelSample.createExcelFileAsByteArray(connection, testFile);
         
         VTradeTab vTradeTab = new VTradeTab(connection);
         vTradeTab.deleteAllRecords();
         vTradeExcelSample.loadDataFromExcelFile(connection, testFile);
         
         //vTradeExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);

         //V_TRADEExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);
   ...
   private byte[] createExcelFileAsByteArray(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"saveDataToExcelFile()\" for VIEW \"V_TRADE\"";
      ...
      try {
         XSSFSheet sheet = workbook.createSheet("V_TRADE");

         if (!vTradeTab.exists()) {
            String errMessage = "VIEW " + vTradeTab.getDbEntityName() + " does not exist.";
            log.error(errMessage);
            throw new GDAOException(errMessage);
         } else {
            log.debug("VIEW " + vTradeTab.getDbEntityName() + " exists.");
         }
          ...
          *
          **********************************************************************/
         //vTradeTab.getResultSet("WHERE UPPER(CREDIT_DEBIT_CODE) LIKE ? AND ROWNUM < ?", "%A%", 100);
          ...
          * 
          **********************************************************************/
         //vTradeTabb.getResultSet(null, (Object[])null);
...
XSSFCell BANK_CODE_cell = rowHeader.createCell(2);
BANK_CODE_cell.setCellValue("BANK CODE");
BANK_CODE_cell.setCellStyle(getStyle("Header", workbook));
...
XSSFCell TRANSACTION_TYPE_cell = rowHeader.createCell(11);
...
XSSFCell TRADE_DATE_cell = rowHeader.createCell(13);
TRADE_DATE_cell.setCellValue("TRADE DATE");
TRADE_DATE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell CREATE_TIMESTAMP_cell = rowHeader.createCell(14);
...
CREATE_TIMESTAMP_cell.setCellStyle(getStyle("Header", workbook));
         ...
         while (vTradeTab.next()) {
            ...
            addNumberCell(workbook, currentRow, column++, row, vTradeTab.getTRADE_ID());
addNumberCell(workbook, currentRow, column++, row, vTradeTab.getBANK_ID());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getBANK_CODE());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getBANK_NAME());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getACCOUNT_ID());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getFIRST_NAME());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getLAST_NAME());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getSECURITY_SYMBOL());
addNumberCell(workbook, currentRow, column++, row, vTradeTab.getNET_AMOUNT());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getCURRENCY());
addNumberCell(workbook, currentRow, column++, row, vTradeTab.getQUANTITY());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getTRANSACTION_TYPE());
addStringCell(workbook, currentRow, column++, row, vTradeTab.getEXCHANGE());
addTimestampCell(workbook, currentRow, column++, row, vTradeTab.getTRADE_DATE());
addTimestampCell(workbook, currentRow, column++, row, vTradeTab.getCREATE_TIMESTAMP());
...
sheet.autoSizeColumn(14);
         ...
         try {
            ...
            e.printStackTrace();  
            log.error("Cannot write workbook to byte[] output stream for VIEW \"V_TRADE\".");
         ...
         try {
            ...
            log.error("Cannot close workbook for VIEW \"V_TRADE\".");
            e.printStackTrace();
            throw new GDAOException(e);
         }
      }
   ...
   private void loadDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException{
      ...
      String whatWeAreDoing = "\"loadDataFromExcelFile()\" for VIEW \"V_TRADE\"";
      log.debug("Starting " + whatWeAreDoing);
         ...
         if (rows > 0 && columns > 0) {
            for (row = 1; row < rows; row++) { // skip first row since it is header row
                  ...
                  int rowColumn = 0;
                        ...
                        vTradeTab.setTRADE_ID(getNumber(currentRow.getCell(0)));
      vTradeTab.setBANK_ID(getNumber(currentRow.getCell(1)));
      vTradeTab.setBANK_CODE(getString(currentRow.getCell(2)));
      vTradeTab.setBANK_NAME(getString(currentRow.getCell(3)));
      vTradeTab.setACCOUNT_ID(getString(currentRow.getCell(4)));
      vTradeTab.setFIRST_NAME(getString(currentRow.getCell(5)));
      vTradeTab.setLAST_NAME(getString(currentRow.getCell(6)));
      vTradeTab.setSECURITY_SYMBOL(getString(currentRow.getCell(7)));
      vTradeTab.setNET_AMOUNT(getNumber(currentRow.getCell(8)));
      vTradeTab.setCURRENCY(getString(currentRow.getCell(9)));
      vTradeTab.setQUANTITY(getNumber(currentRow.getCell(10)));
      vTradeTab.setTRANSACTION_TYPE(getString(currentRow.getCell(11)));
      vTradeTab.setEXCHANGE(getString(currentRow.getCell(12)));
      vTradeTab.setTRADE_DATE(getTimestamp(currentRow.getCell(13)));
      vTradeTab.setCREATE_TIMESTAMP(getTimestamp(currentRow.getCell(14)));
         ...
         long rowCount = vTradeTab.getRowCount();

         log.info("Inserted Records in VIEW V_TRADE DB: " + inserted);
         log.info("Total errors: " + errors);
         log.info("Total row count in VIEW \"V_TRADE\" is " + rowCount);
   ...
   private void updateOrInsertDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"updateOrInsertDataFromExcelFile()\" for VIEW \"V_TRADE\"";
      log.debug("Starting " + whatWeAreDoing);
      ...
      vTradeTab.setCREATE_TIMESTAMP(getTimestamp(currentRow.getCell(14)));
                ...
                **********************************************************************/
               try {
                  recordsUpdated = vTradeTab.updateThisRecord();
               } catch (GDAOException e) {
         ...
         log.info("Inserted Records in VIEW V_TRADE DB: " + inserted);
         log.info("Updated Records in VIEW V_TRADE DB: " + updated);
         log.info("Total errors: " + errors);
         log.info("Row Count (VIEW V_TRADE) =" + rowCount);
