 ...
 *  <pre>
 *
 *  Generated Class: USERSExcelSample.java
 * 
...
public class UsersExcelSample {

   final static Logger log = Logger.getLogger(UsersExcelSample.class.getName());

   final static String ENTITY_NAME = "USERS";
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         UsersExcelSample usersExcelSample = new UsersExcelSample();
          ...
          *
          **********************************************************************/
         usersExcelSample.createExcelFileAsByteArray(connection, testFile);
         
         UsersTab usersTab = new UsersTab(connection);
         usersTab.deleteAllRecords();
         usersExcelSample.loadDataFromExcelFile(connection, testFile);
         
         //usersExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);
   ...
   private byte[] createExcelFileAsByteArray(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"saveDataToExcelFile()\" for TABLE \"USERS\"";
      ...
      try {
         ...
         if (!usersTab.exists()) {
            String errMessage = "TABLE " + usersTab.getDbEntityName() + " does not exist.";
            log.error(errMessage);
            throw new GDAOException(errMessage);
         } else {
            log.debug("TABLE " + usersTab.getDbEntityName() + " exists.");
         }
          ...
          *
          **********************************************************************/
         //usersTab.getResultSet("WHERE UPPER(CREDIT_DEBIT_CODE) LIKE ? AND ROWNUM < ?", "%A%", 100);
          ...
          * 
          **********************************************************************/
         //usersTabb.getResultSet(null, (Object[])null);
         ...
         XSSFCell USER_ID_cell = rowHeader.createCell(0);
USER_ID_cell.setCellValue("USER ID");
USER_ID_cell.setCellStyle(getStyle("Header", workbook));
...
XSSFCell USER_NAME_cell = rowHeader.createCell(2);
USER_NAME_cell.setCellValue("USER NAME");
USER_NAME_cell.setCellStyle(getStyle("Header", workbook));
         ...
         while (usersTab.next()) {
            ...
            addNumberCell(workbook, currentRow, column++, row, usersTab.getUSER_ID());
addNumberCell(workbook, currentRow, column++, row, usersTab.getBANK_ID());
addStringCell(workbook, currentRow, column++, row, usersTab.getUSER_NAME());
...
sheet.autoSizeColumn(2);
         ...
         try {
            ...
            e.printStackTrace();  
            log.error("Cannot write workbook to byte[] output stream for TABLE \"USERS\".");
         ...
         try {
            ...
            log.error("Cannot close workbook for TABLE \"USERS\".");
            e.printStackTrace();
            throw new GDAOException(e);
         }
      }
   ...
   private void loadDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException{
      ...
      String whatWeAreDoing = "\"loadDataFromExcelFile()\" for TABLE \"USERS\"";
      log.debug("Starting " + whatWeAreDoing);
         ...
         if (rows > 0 && columns > 0) {
            for (row = 1; row < rows; row++) { // skip first row since it is header row
                  ...
                  int rowColumn = 0;
                        ...
                        usersTab.setUSER_ID(getNumber(currentRow.getCell(0)));
      usersTab.setBANK_ID(getNumber(currentRow.getCell(1)));
      usersTab.setUSER_NAME(getString(currentRow.getCell(2)));
         ...
         log.info("Inserted Records in TABLE USERS DB: " + inserted);
         log.info("Total errors: " + errors);
         log.info("Total row count in TABLE \"USERS\" is " + rowCount);
   ...
   private void updateOrInsertDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"updateOrInsertDataFromExcelFile()\" for TABLE \"USERS\"";
      log.debug("Starting " + whatWeAreDoing);
      ...
      usersTab.setUSER_NAME(getString(currentRow.getCell(2)));
                ...
                **********************************************************************/
               try {
                  recordsUpdated = usersTab.updateThisRecord();
               } catch (GDAOException e) {
         ...
         log.info("Inserted Records in TABLE USERS DB: " + inserted);
         log.info("Updated Records in TABLE USERS DB: " + updated);
         log.info("Total errors: " + errors);
         log.info("Row Count (TABLE USERS) =" + rowCount);
