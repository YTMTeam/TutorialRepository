 ...
 *  <pre>
 *
 *  Generated Class: ACCOUNTExcelSample.java
 * 
...
public class AccountExcelSample {

   final static Logger log = Logger.getLogger(AccountExcelSample.class.getName());

   final static String ENTITY_NAME = "ACCOUNT";
   ...
   public static void main(String[] args) {
      ...
      try {
         ...
         AccountExcelSample accountExcelSample = new AccountExcelSample();
          ...
          *
          **********************************************************************/
         accountExcelSample.createExcelFileAsByteArray(connection, testFile);
         
         AccountTab accountTab = new AccountTab(connection);
         accountTab.deleteAllRecords();
         accountExcelSample.loadDataFromExcelFile(connection, testFile);
         
         //accountExcelSample.updateOrInsertDataFromExcelFile(connection, testFile);
   ...
   public byte[] createExcelFileAsByteArray(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"saveDataToExcelFile()\" for TABLE \"ACCOUNT\"";
...
XSSFCell MIDDLE_NAME_cell = rowHeader.createCell(4);
...
XSSFCell BROKER_CODE_cell = rowHeader.createCell(7);
BROKER_CODE_cell.setCellValue("BROKER CODE");
BROKER_CODE_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell MANAGED_ACCOUNT_cell = rowHeader.createCell(8);
MANAGED_ACCOUNT_cell.setCellValue("MANAGED ACCOUNT");
MANAGED_ACCOUNT_cell.setCellStyle(getStyle("Header", workbook));

XSSFCell ACCOUNT_STATUS_cell = rowHeader.createCell(9);
ACCOUNT_STATUS_cell.setCellValue("ACCOUNT STATUS");
ACCOUNT_STATUS_cell.setCellStyle(getStyle("Header", workbook));
         ...
         while (accountTab.next()) {
            ...
            addStringCell(workbook, currentRow, column++, row, accountTab.getACCOUNT_ID());
addNumberCell(workbook, currentRow, column++, row, accountTab.getBANK_ID());
addNumberCell(workbook, currentRow, column++, row, accountTab.getACCOUNT_NUMBER());
addStringCell(workbook, currentRow, column++, row, accountTab.getFIRST_NAME());
addStringCell(workbook, currentRow, column++, row, accountTab.getMIDDLE_NAME());
addStringCell(workbook, currentRow, column++, row, accountTab.getLAST_NAME());
addStringCell(workbook, currentRow, column++, row, accountTab.getTYPE());
addStringCell(workbook, currentRow, column++, row, accountTab.getBROKER_CODE());
addStringCell(workbook, currentRow, column++, row, accountTab.getMANAGED_ACCOUNT());
addStringCell(workbook, currentRow, column++, row, accountTab.getACCOUNT_STATUS());
...
sheet.autoSizeColumn(9);
         ...
         try {
            ...
            e.printStackTrace();  
            log.error("Cannot write workbook to byte[] output stream for TABLE \"ACCOUNT\".");
         ...
         try {
            ...
            log.error("Cannot close workbook for TABLE \"ACCOUNT\".");
            e.printStackTrace();
            throw new GDAOException(e);
         }
      }
   ...
   private void loadDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException{
      ...
      String whatWeAreDoing = "\"loadDataFromExcelFile()\" for TABLE \"ACCOUNT\"";
      log.debug("Starting " + whatWeAreDoing);
         ...
         if (rows > 0 && columns > 0) {
            for (row = 1; row < rows; row++) { // skip first row since it is header row
                  ...
                  int rowColumn = 0;
                        ...
                        accountTab.setACCOUNT_ID(getString(currentRow.getCell(0)));
      accountTab.setBANK_ID(getNumber(currentRow.getCell(1)));
      accountTab.setACCOUNT_NUMBER(getNumber(currentRow.getCell(2)));
      accountTab.setFIRST_NAME(getString(currentRow.getCell(3)));
      accountTab.setMIDDLE_NAME(getString(currentRow.getCell(4)));
      accountTab.setLAST_NAME(getString(currentRow.getCell(5)));
      accountTab.setTYPE(getString(currentRow.getCell(6)));
      accountTab.setBROKER_CODE(getString(currentRow.getCell(7)));
      accountTab.setMANAGED_ACCOUNT(getString(currentRow.getCell(8)));
      accountTab.setACCOUNT_STATUS(getString(currentRow.getCell(9)));
         ...
         log.info("Inserted Records in TABLE ACCOUNT DB: " + inserted);
         log.info("Total errors: " + errors);
         log.info("Total row count in TABLE \"ACCOUNT\" is " + rowCount);
   ...
   private void updateOrInsertDataFromExcelFile(Connection connection, String excelFileName) throws GDAOException {
      ...
      String whatWeAreDoing = "\"updateOrInsertDataFromExcelFile()\" for TABLE \"ACCOUNT\"";
      log.debug("Starting " + whatWeAreDoing);
      ...
      accountTab.setACCOUNT_STATUS(getString(currentRow.getCell(9)));
         ...
         log.info("Inserted Records in TABLE ACCOUNT DB: " + inserted);
         log.info("Updated Records in TABLE ACCOUNT DB: " + updated);
         log.info("Total errors: " + errors);
