 ...
 *  <pre>
 *
 *  Generated Class: VTradeDBAdminSample.java
 * 
...
public class VTradeDBAdminSample {

   final static Logger log = Logger.getLogger(VTradeDBAdminSample.class.getName());
   ...
   public static void main(String[] args) {
     ...
     try {
        ...
        *
        **********************************************************************/
        VTradeDBAdminSample vTradeDBAdminSample = new VTradeDBAdminSample();
         ...
         *
         **********************************************************************/
         //vTradeDBAdminSample.backupTable(connection);
         //vTradeDBAdminSample.restoreTable(connection);
         //vTradeDBAdminSample.restoreTableInOneStatement(connection);
         
         //vTradeDBAdminSample.showVolumesInformation(connection);
         //vTradeDBAdminSample.showMetaDataInformation(connection);
   ...
   **********************************************************************/
   public void backupTable(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"backupTable()\" for VIEW \"V_TRADE\"";
   
      String SQL = null;
      OracleSystemTab oracleSystemTab = new OracleSystemTab(connection);
      log.debug("Backing up VIEW \"V_TRADE\"...");
      try {
         /**********************************************************************
         *
         ...
         *
         **********************************************************************/
         SQL = "CREATE TABLE V_TRADE_BKP AS SELECT * FROM V_TRADE";
         oracleSystemTab.runSQL(SQL);
      } catch (GDAOException e) {
         e.printStackTrace();
         log.error("Error occurred while executing SQL statement:" + SQL);
         log.error("Cannot perform DDL on VIEW \"V_TRADE\".");
         throw new GDAOException(e);
      }

      log.debug("VIEW \"V_TRADE\" is backed up.");
   }

   /**********************************************************************
   *
   * This method demonstrates usage of plain SQL by restoring
   * VIEW "V_TRADE" from the backup
   *
   * @throws GDAOException
   *
   **********************************************************************/
   public void restoreTableInOneStatement(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"restoreTable()\" for VIEW \"V_TRADE\"";
   
      String SQL = null;
      OracleSystemTab oracleSystemTab = new OracleSystemTab(connection);
      
      log.debug("Restoring VIEW \"V_TRADE\"...");
      ...
      try {
         ...
         **********************************************************************/
         ...
         SQL = "INSERT INTO V_TRADE (SELECT * FROM V_TRADE_BKP)";
         oracleSystemTab.runSQL(SQL);
      } catch (GDAOException e) {
         e.printStackTrace();
         log.error("Error occurred while executing SQL statement:" + SQL);
      ...
      log.debug("VIEW \"V_TRADE\" is restored from the backup.");
   }
   ...
   *     VIEW \"V_TRADE\" from the backup table 
   *     V_TRADE_BKP (see description below).
   ...
   **********************************************************************/
   public void restoreTable(Connection connection) throws GDAOException {
      ...
      log.info("Restoring VIEW \"V_TRADE\"...");
      try {
         ...
         **********************************************************************/
         ...
         if (rowCount > TRANSACTION_STRATEGY_TRESHOLD ) {
            ...
            while (vTradeTab.next()) {
               ...
               /**********************************************************************
                *
                * Insert record in table V_TRADE
                * using the same DAO object "vTradeTab"
                *
                **********************************************************************/
            ...
            log.info("VIEW \"V_TRADE\" has only "
                     + rowCount
      ...
      log.info("VIEW \"V_TRADE\" is restored from the backup TABLE V_TRADE_BKP.");
   }
   ...
   public void showVolumesInformation(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"showVolumesInformation()\" for VIEW \"V_TRADE\"";
      ...
      try {
          ...
          if (vTradeTab.exists()) {
             log.info("VIEW \"V_TRADE\" exists in the database.");
          } else {
             log.info("VIEW \"V_TRADE\" does not exist in the database.");
             return;
          }
          ...
          *
          **********************************************************************/
         log.info("VIEW \"V_TRADE\" row count=" + vTradeTab.getRowCount());
         ...
         *
         **********************************************************************/
         log.info("ResultSet Row Count=" + vTradeTab.getResultSetRowCount());
   ...
   **********************************************************************/
   public void showMetaDataInformation(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"showMetaDataInformation()\" for VIEW \"V_TRADE\"";
      ...
      try {
         ...
         **********************************************************************/
          ...
          *
          **********************************************************************/
         tablesResultSet = databaseMetaData.getTables(null, null, "V_TRADE", null);
          ...
          *
          **********************************************************************/
         columnsResultSet = databaseMetaData.getColumns(null, null, "V_TRADE", null);
         ...
         **********************************************************************/
         ...
         e.printStackTrace();
         log.error("Cannot get meta data for VIEW \"V_TRADE\".");
         throw new GDAOException(e);
      } finally {
