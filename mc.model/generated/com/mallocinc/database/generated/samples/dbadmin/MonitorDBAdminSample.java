 ...
 *  <pre>
 *
 *  Generated Class: MonitorDBAdminSample.java
 * 
...
public class MonitorDBAdminSample {

   final static Logger log = Logger.getLogger(MonitorDBAdminSample.class.getName());
   ...
   public static void main(String[] args) {
     ...
     try {
        ...
        *
        **********************************************************************/
        MonitorDBAdminSample monitorDBAdminSample = new MonitorDBAdminSample();
         ...
         *
         **********************************************************************/
         //monitorDBAdminSample.backupTable(connection);
         //monitorDBAdminSample.restoreTable(connection);
         //monitorDBAdminSample.restoreTableInOneStatement(connection);
         
         //monitorDBAdminSample.showVolumesInformation(connection);
         //monitorDBAdminSample.showMetaDataInformation(connection);
   ...
   **********************************************************************/
   public void backupTable(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"backupTable()\" for TABLE \"MONITOR\"";
   
      String SQL = null;
      OracleSystemTab oracleSystemTab = new OracleSystemTab(connection);
      log.debug("Backing up TABLE \"MONITOR\"...");
      try {
         /**********************************************************************
         *
         ...
         *
         **********************************************************************/
         SQL = "CREATE TABLE MONITOR_BKP AS SELECT * FROM MONITOR";
         oracleSystemTab.runSQL(SQL);
      } catch (GDAOException e) {
         e.printStackTrace();
         log.error("Error occurred while executing SQL statement:" + SQL);
         log.error("Cannot perform DDL on TABLE \"MONITOR\".");
         throw new GDAOException(e);
      }

      log.debug("TABLE \"MONITOR\" is backed up.");
   }

   /**********************************************************************
   *
   * This method demonstrates usage of plain SQL by restoring
   * TABLE "MONITOR" from the backup
   *
   * @throws GDAOException
   *
   **********************************************************************/
   public void restoreTableInOneStatement(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"restoreTable()\" for TABLE \"MONITOR\"";
   
      String SQL = null;
      OracleSystemTab oracleSystemTab = new OracleSystemTab(connection);
      
      log.debug("Restoring TABLE \"MONITOR\"...");
      ...
      try {
         ...
         **********************************************************************/
         ...
         SQL = "INSERT INTO MONITOR (SELECT * FROM MONITOR_BKP)";
         oracleSystemTab.runSQL(SQL);
      } catch (GDAOException e) {
         e.printStackTrace();
         log.error("Error occurred while executing SQL statement:" + SQL);
      ...
      log.debug("TABLE \"MONITOR\" is restored from the backup.");
   }
   ...
   *     TABLE \"MONITOR\" from the backup table 
   *     MONITOR_BKP (see description below).
   ...
   **********************************************************************/
   public void restoreTable(Connection connection) throws GDAOException {
      ...
      log.info("Restoring TABLE \"MONITOR\"...");
      try {
         ...
         **********************************************************************/
         ...
         if (rowCount > TRANSACTION_STRATEGY_TRESHOLD ) {
            ...
            while (monitorTab.next()) {
               ...
               /**********************************************************************
                *
                * Insert record in table MONITOR
                * using the same DAO object "monitorTab"
                *
                **********************************************************************/
            ...
            log.info("TABLE \"MONITOR\" has only "
                     + rowCount
      ...
      log.info("TABLE \"MONITOR\" is restored from the backup TABLE MONITOR_BKP.");
   }
   ...
   public void showVolumesInformation(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"showVolumesInformation()\" for TABLE \"MONITOR\"";
      ...
      try {
          ...
          if (monitorTab.exists()) {
             log.info("TABLE \"MONITOR\" exists in the database.");
          } else {
             log.info("TABLE \"MONITOR\" does not exist in the database.");
             return;
          }
          ...
          *
          **********************************************************************/
         log.info("TABLE \"MONITOR\" row count=" + monitorTab.getRowCount());
         ...
         *
         **********************************************************************/
         log.info("ResultSet Row Count=" + monitorTab.getResultSetRowCount());
   ...
   **********************************************************************/
   public void showMetaDataInformation(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"showMetaDataInformation()\" for TABLE \"MONITOR\"";
      ...
      try {
         ...
         **********************************************************************/
          ...
          *
          **********************************************************************/
         tablesResultSet = databaseMetaData.getTables(null, null, "MONITOR", null);
          ...
          *
          **********************************************************************/
         columnsResultSet = databaseMetaData.getColumns(null, null, "MONITOR", null);
         ...
         **********************************************************************/
         ...
         e.printStackTrace();
         log.error("Cannot get meta data for TABLE \"MONITOR\".");
         throw new GDAOException(e);
      } finally {
