 ...
 *  <pre>
 *
 *  Generated Class: TransactionDBAdminSample.java
 * 
...
public class TransactionDBAdminSample {

   final static Logger log = Logger.getLogger(TransactionDBAdminSample.class.getName());
   ...
   public static void main(String[] args) {
     ...
     try {
        ...
        *
        **********************************************************************/
        TransactionDBAdminSample transactionDBAdminSample = new TransactionDBAdminSample();
         ...
         *
         **********************************************************************/
         //transactionDBAdminSample.backupTable(connection);
         //transactionDBAdminSample.restoreTable(connection);
         //transactionDBAdminSample.restoreTableInOneStatement(connection);
         
         //transactionDBAdminSample.showVolumesInformation(connection);
         //transactionDBAdminSample.showMetaDataInformation(connection);
   ...
   **********************************************************************/
   public void backupTable(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"backupTable()\" for TABLE \"TRANSACTION\"";
   
      String SQL = null;
      OracleSystemTab oracleSystemTab = new OracleSystemTab(connection);
      log.debug("Backing up TABLE \"TRANSACTION\"...");
      try {
         /**********************************************************************
         *
         ...
         *
         **********************************************************************/
         SQL = "DROP TABLE TRANSACTION_BKP";
         ...
         *
         **********************************************************************/
         SQL = "CREATE TABLE TRANSACTION_BKP AS SELECT * FROM TRANSACTION";
         oracleSystemTab.runSQL(SQL);
      } catch (GDAOException e) {
         e.printStackTrace();
         log.error("Error occurred while executing SQL statement:" + SQL);
         log.error("Cannot perform DDL on TABLE \"TRANSACTION\".");
         throw new GDAOException(e);
      }

      log.debug("TABLE \"TRANSACTION\" is backed up.");
   }

   /**********************************************************************
   *
   * This method demonstrates usage of plain SQL by restoring
   * TABLE "TRANSACTION" from the backup
   *
   * @throws GDAOException
   *
   **********************************************************************/
   public void restoreTableInOneStatement(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"restoreTable()\" for TABLE \"TRANSACTION\"";
   
      String SQL = null;
      OracleSystemTab oracleSystemTab = new OracleSystemTab(connection);
      
      log.debug("Restoring TABLE \"TRANSACTION\"...");
      ...
      try {
         ...
         **********************************************************************/
         ...
         SQL = "INSERT INTO TRANSACTION (SELECT * FROM TRANSACTION_BKP)";
         oracleSystemTab.runSQL(SQL);
      } catch (GDAOException e) {
         e.printStackTrace();
         log.error("Error occurred while executing SQL statement:" + SQL);
      ...
      log.debug("TABLE \"TRANSACTION\" is restored from the backup.");
   }
   ...
   *     TABLE \"TRANSACTION\" from the backup table 
   *     TRANSACTION_BKP (see description below).
   ...
   **********************************************************************/
   public void restoreTable(Connection connection) throws GDAOException {
      ...
      log.info("Restoring TABLE \"TRANSACTION\"...");
      try {
         ...
         **********************************************************************/
         ...
         if (rowCount > TRANSACTION_STRATEGY_TRESHOLD ) {
            ...
            while (transactionTab.next()) {
                ...
                * using DAO object "transactionTab"
                *
                **********************************************************************/
               ...
               /**********************************************************************
                *
                * Insert record in table TRANSACTION
                * using the same DAO object "transactionTab"
                *
                **********************************************************************/
            ...
            log.info("TABLE \"TRANSACTION\" has only "
                     + rowCount
      ...
      log.info("TABLE \"TRANSACTION\" is restored from the backup TABLE TRANSACTION_BKP.");
   }
   ...
   public void showVolumesInformation(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"showVolumesInformation()\" for TABLE \"TRANSACTION\"";
      ...
      try {
          ...
          if (transactionTab.exists()) {
             log.info("TABLE \"TRANSACTION\" exists in the database.");
          } else {
             log.info("TABLE \"TRANSACTION\" does not exist in the database.");
             return;
          }
          ...
          *
          **********************************************************************/
         log.info("TABLE \"TRANSACTION\" row count=" + transactionTab.getRowCount());
         ...
         *
         **********************************************************************/
         log.info("ResultSet Row Count=" + transactionTab.getResultSetRowCount());
   ...
   **********************************************************************/
   public void showMetaDataInformation(Connection connection) throws GDAOException {
      ...
      String whatWeAreDoing = "\"showMetaDataInformation()\" for TABLE \"TRANSACTION\"";
      ...
      try {
         ...
         **********************************************************************/
          ...
          *
          **********************************************************************/
         tablesResultSet = databaseMetaData.getTables(null, null, "TRANSACTION", null);
          ...
          *
          **********************************************************************/
         columnsResultSet = databaseMetaData.getColumns(null, null, "TRANSACTION", null);
         ...
         **********************************************************************/
         ...
         e.printStackTrace();
         log.error("Cannot get meta data for TABLE \"TRANSACTION\".");
         throw new GDAOException(e);
      } finally {
